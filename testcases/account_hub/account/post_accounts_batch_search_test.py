...
# FROM: .\testcases\account_hub\account\post_accounts_batch_search.yml
import pytest
...
class TestCasePostAccountsBatchSearch(HttpRunner):
        ...
        Parameters(
            {
                "name-code-status_code-msg": [
                    ...
                    [
                        "批量拉取用户信息-通过phone_number 进行查询 查询多个phone_number",
                        14,
                        200,
                        "created_at",
                    ],
    ...
    config = (
        Config("/accounts/batch/search 批量拉取用户信息")
    ...
    teststeps = [
        Step(
            RunRequest("$name")
            .setup_hook("${account_batch_search($request,$code)}")
            .post("/accounts/batch/search")
            .validate()
...
if __name__ == "__main__":
    TestCasePostAccountsBatchSearch().test_start()
